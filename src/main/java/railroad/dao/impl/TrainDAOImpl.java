...
public class TrainDAOImpl implements TrainDAO {
    ...
    public int countAllTrains() {
        return sessionFactory.getCurrentSession()
                .createQuery("SELECT t.id FROM Train AS t").list().size();
    }
    ...
    public List<Integer> getTrainNumberList(int page, int onPage) {
        return sessionFactory.getCurrentSession()
                .createQuery("SELECT t.number FROM Train AS t")
                .setFirstResult(onPage * (page - 1)).setMaxResults(onPage).list();
    }
    ...
    public int countByStationName(String stationName) {
        return sessionFactory.getCurrentSession()
                .createQuery("SELECT t.id FROM Train AS t INNER JOIN StationTrain AS st ON t.id = st.train_id INNER JOIN Station AS s ON st.station_id = s.id WHERE s.station_name = :stationName")
    ...
    public List<Integer> getIdByStationNameList(String stationName, int page, int onPage) {
        return sessionFactory.getCurrentSession()
                .createQuery("SELECT t.id FROM Train AS t INNER JOIN StationTrain AS st ON t.id = st.train_id INNER JOIN Station AS s ON st.station_id = s.id WHERE s.station_name = :stationName ORDER BY st.time")
                .setParameter("stationName", stationName).setFirstResult(onPage * (page - 1)).setMaxResults(onPage).list();
    }
    ...
    public int getTrainNumberByIdSingle(int id) {
        return sessionFactory.getCurrentSession()
                .createQuery("SELECT t.number FROM Train AS t WHERE t.id = :id", Number.class)
    ...
    public List<Integer> getIdByArrivalSearchList(String arrivalStationName, Time lowerTime, Time upperTime) {
        return sessionFactory.getCurrentSession()
                .createQuery("SELECT t.id FROM Train AS t INNER JOIN StationTrain AS st ON t.id = st.train_id INNER JOIN Station AS s ON st.station_id = s.id WHERE s.station_name = :arrivalStationName AND st.time > :lowerTime AND st.time < :upperTime")
                .setParameter("arrivalStationName", arrivalStationName)
                .setParameter("lowerTime", lowerTime)
                .setParameter("upperTime", upperTime)
                .list();
    }
    ...
    public int isAppropriateBySearch(int train_id, String departureStationName, Time untilTime) {
        return sessionFactory.getCurrentSession()
                .createQuery("SELECT t.id FROM Train AS t INNER JOIN StationTrain AS st ON t.id = st.train_id INNER JOIN Station AS s ON st.station_id = s.id WHERE t.id = :train_id AND s.station_name = :departureStationName AND st.time < :untilTime")
                .setParameter("train_id", train_id)
                .setParameter("departureStationName", departureStationName)
    ...
    public int getSeatsByIdSingle(int id) {
        return sessionFactory.getCurrentSession()
                .createQuery("SELECT t.seats FROM Train AS t WHERE t.id = :trainID", Number.class)
                .setParameter("trainID", id).getSingleResult().intValue();
    }
    ...
    public int countByTrainNumber(int trainNumber) {
        return sessionFactory.getCurrentSession()
                .createQuery("SELECT t.id FROM Train AS t WHERE t.number = :trainNumber")
    ...
    public int getIdByTrainNumberSingle(int trainNumber) {
        return sessionFactory.getCurrentSession()
                .createQuery("SELECT t.id FROM Train AS t WHERE t.number = :trainNumber", Number.class)
                .setParameter("trainNumber", trainNumber).getSingleResult().intValue();
    }
    ...
    public int getTrainNumberByTicketIdSingle(int id) {
        return sessionFactory.getCurrentSession()
                .createQuery("SELECT t.number FROM Ticket AS i INNER JOIN Train AS t ON i.train_id = t.id WHERE i.id = :id", Number.class)
                .setParameter("id", id).getSingleResult().intValue();
    }
    ...
    public void deleteByTrainNumber(int trainNumber) {
        sessionFactory.getCurrentSession()
                .createQuery("DELETE FROM Train AS t WHERE t.number = :trainNumber")
                .setParameter("trainNumber", trainNumber).executeUpdate();
    }
